...
/**
 * Sistema de Rate Limiting - 5 requisições por minuto
 */
...
async function enforceRateLimit(subdomain: string): Promise<void> {
	...
	if (entry.timestamps.length >= RATE_LIMIT_MAX_REQUESTS) {
		...
		console.log(`[Rate Limit] Limite de ${RATE_LIMIT_MAX_REQUESTS} req/min excedido para ${subdomain}. Aguardando ${Math.ceil(waitTime / 1000)}s...`);
